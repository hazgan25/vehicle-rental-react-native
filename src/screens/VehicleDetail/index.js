...
import { putVehicleById } from '../../modules/utils/vehicles'
...
const VehicleDetail = ({ navigation, route }) => {
    ...
    const updateItemHandler = () => {
        ...
        bodyUpdate.append('id', vehicleId)
        ...
        putVehicleById(bodyUpdate, token)
            .then((res) => {
                console.log(res)
                if (res.result.err) {
                    Toast.show({
    ...
    console.log(paymentStock > vehicleStock)
    const btnMin = () => {
    ...
    return (
        <ScrollView style={{ backgroundColor: '#fff' }}>
            <View style={{
                height: !isEdit ? height + 120 : height + 170,
                width: width,
                flex: 1
            }}>
                        ...
                        images.map((data, idx) => (
                            <React.Fragment key={idx}>
                                <TouchableOpacity disabled={owner_id !== id ? true : !isEdit ? true : false} onPress={browserGalery}>
                                    <Image source={
                                        !`${process.env.HOST}/${data.images}` ? defaultVehicleImg :
                                            !{ uri: `${process.env.HOST}/${data.images}` } ? defaultVehicleImg :
                                                { uri: `${process.env.HOST}/${data.images}` }
                                    }
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                        {owner_id === id ? (
                            <>
                                ...
                                <TouchableOpacity style={styles.trashBg}>
